...
class UltralyticsRuntimeTest(unittest.TestCase):
    ...
    def test_bootstrap_sets_config_dir_before_import(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            repeated = bootstrap_ultralytics_config(root, environ=environment)

            self.assertEqual(environment["YOLO_CONFIG_DIR"], str(path))
            self.assertEqual(repeated, path)
            self.assertNotEqual(path, root)
            self.assertNotIn(root, path.parents)
            self.assertTrue(path.is_dir())

    def test_bootstrap_preserves_registered_external_config_dir(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            external = root.parent / f"{root.name}-runtime" / "yolo-config"
            environment = {"YOLO_CONFIG_DIR": str(external)}

            path = bootstrap_ultralytics_config(root, environ=environment)

            self.assertEqual(path, external.resolve())
            self.assertEqual(environment["YOLO_CONFIG_DIR"], str(external.resolve()))
            self.assertTrue(path.is_dir())
