...
class IFDRDetectionModelTest(unittest.TestCase):
    ...
    def test_default_p2_path_switches_are_registered_and_identity_stable(self) -> None:
        from ifdr_yolo.models.ifdr_model import P2PathSwitches

        self.assertEqual(
            self.ifdr.p2_path_switches,
            P2PathSwitches(
                nodes=(17,),
        ...
        self.assertTrue(self.ifdr.model[17].fusion_modulation_enabled)
        self.assertEqual(
            self.ifdr.p2_path_switches.identity,
            {
                "nodes": (17,),
                "fusion_modulation": True,
                "dcli_factor_conditioning": True,
    ...
    def test_s1_only_disables_node17_fusion_modulation_and_keeps_detect(self) -> None:
        from ifdr_yolo.models.gated_fusion import ReliabilityGatedConcat
        from ifdr_yolo.models.ifdr_model import P2PathSwitches

        layer = self.ifdr.model[17]
        self.assertIsInstance(layer, ReliabilityGatedConcat)
        assert isinstance(layer, ReliabilityGatedConcat)
        layer.set_schedule(1.0)
        inputs = (
            torch.randn(1, 192, 32, 32),
            torch.randn(1, 96, 32, 32),
        )
        enabled = layer(inputs)
        enabled_context = layer.consume_context()
        self.ifdr.set_p2_path_switches(
            P2PathSwitches(
                nodes=(17,),
        ...
        disabled_context = layer.consume_context()

        self.assertTrue(torch.equal(disabled, torch.cat(inputs, dim=1)))
        self.assertFalse(torch.equal(enabled, disabled))
        self.assertTrue(torch.equal(enabled_context.factors, disabled_context.factors))
        self.assertEqual(disabled.shape[1], 288)

        self.ifdr.train()
        with torch.no_grad():
            output = self.ifdr(torch.zeros(1, 3, 128, 128))
        self.assertEqual(set(output), {"boxes", "scores", "feats"})
        self.assertEqual(tuple(self.ifdr.consume_reliability_context()), self.ifdr.fusion_node_indices)

    def test_s3_disables_both_registered_node17_paths(self) -> None:
        from ifdr_yolo.models.ifdr_model import P2PathSwitches
        ...
        self.assertFalse(self.ifdr.p2_path_switches.fusion_modulation)
        self.assertFalse(self.ifdr.p2_path_switches.dcli_factor_conditioning)
        self.assertFalse(self.ifdr.model[17].fusion_modulation_enabled)
        self.assertEqual(self.ifdr.fusion_node_indices, (11, 14, 17, 20, 23, 26))

    def test_rejects_unregistered_or_empty_p2_path_nodes(self) -> None:
        from ifdr_yolo.models.ifdr_model import IFDRDetectionModel

        for nodes in ((), (20,), (17, 20), None):
            with self.subTest(nodes=nodes):
                with self.assertRaisesRegex(ValueError, "registered node 17"):
                    IFDRDetectionModel(
                        str(MODEL_PATH),
                        verbose=False,
                        p2_path_switches={
