...
class IFDRP2PathSwitchConfigTest(unittest.TestCase):
    def test_default_ifdr_config_binds_registered_p2_switch_identity(self) -> None:
        config = load_ifdr_config(
            ROOT / "configs/experiments/kitti_ifdr_yolov8m_s17.yaml",
            repository_root=ROOT,
        )

        switches = config.method.p2_path_switches
        self.assertEqual(switches.nodes, (17,))
        self.assertTrue(switches.fusion_modulation)
        self.assertTrue(switches.dcli_factor_conditioning)
        self.assertEqual(
            switches.identity,
            {
                "nodes": (17,),
    ...
    def test_rejects_unregistered_or_empty_p2_path_switch_nodes(self) -> None:
        source = ROOT / "configs/experiments/kitti_ifdr_yolov8m_s17.yaml"
        for nodes in ([], [20], [17, 20], None):
            with self.subTest(nodes=nodes):
                payload = yaml.safe_load(source.read_text(encoding="utf-8"))
                payload["ifdr"]["p2_path_switches"] = {
                    "nodes": nodes,
                    "fusion_modulation": True,
                    "dcli_factor_conditioning": True,
                }
                with tempfile.TemporaryDirectory() as directory:
                    path = Path(directory) / "invalid-p2-switches.yaml"
                    path.write_text(
