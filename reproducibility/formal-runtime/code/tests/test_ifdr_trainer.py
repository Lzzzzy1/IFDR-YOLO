...
class IFDRDetectionTrainerTest(unittest.TestCase):
    def test_flush_gradient_diagnostics_appends_jsonl_records(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "ifdr_yolo.experiments.ifdr_trainer.os.getpid",
                side_effect=(101, 202),
            ):
            ...
            self.assertEqual([payload["epoch"] for payload in payloads], [5, 5])
            self.assertEqual(
                [payload["process_id"] for payload in payloads],
                [101, 202],
            )
            self.assertEqual(payloads[0]["step"], payloads[1]["step"])
            self.assertEqual(record, original_record)
            self.assertNotIn("epoch", record)
            self.assertNotIn("process_id", record)

    def test_flush_gradient_diagnostics_rejects_invalid_epoch(self) -> None:
        from ifdr_yolo.experiments.ifdr_trainer import (
            flush_gradient_diagnostics,
        )
    ...
    def test_get_model_builds_project_owned_detector(self) -> None:
        ...
        trainer.gradient_diagnostic_interval = 50
        ...
        self.assertEqual(model._gradient_diagnostics.interval, 50)
