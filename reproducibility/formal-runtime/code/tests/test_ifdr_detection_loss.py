...
class DCLIBboxLossTest(unittest.TestCase):
    ...
    def test_observer_components_separate_base_conditioning_and_calibration(self) -> None:
        from ifdr_yolo.losses.ifdr_detection import DCLIBboxLoss
        ...
        uncertainty = torch.tensor([[0.3, 0.9]], requires_grad=True)
        dcli.set_uncertainty(uncertainty)
        inputs = list(self._inputs())
        inputs[1].requires_grad_(True)
        dcli(*inputs)

        components = dcli.consume_diagnostic_components()

        self.assertEqual(
            set(components),
            {"base_localization", "dcli_conditioning", "dcli_calibration"},
        )
...
class IFDRDetectionLossIntegrationTest(unittest.TestCase):
    def test_s2_excludes_node17_factors_but_keeps_dfl_entropy_conditioning(
        self,
    ) -> None:
        from ultralytics.utils import DEFAULT_CFG

        import ifdr_yolo.losses.ifdr_detection as detection_loss
        ...
        original = detection_loss.derive_localization_uncertainty

        def capture(factors, dfl_entropy, **kwargs):
            captured["factors"] = factors.detach().clone()
            captured["dfl_entropy"] = dfl_entropy.detach().clone()
            return original(factors, dfl_entropy, **kwargs)
        ...
        self.assertIn("factors", captured)
        factors = captured["factors"]
        entropy = captured["dfl_entropy"]
        p2_count = 32 * 32
        expected = entropy[:, :p2_count].unsqueeze(-1).expand(-1, -1, 2)
        self.assertTrue(torch.equal(factors[:, :p2_count], expected))

    def test_node_diagnostic_components_reconstruct_detection_scalar(self) -> None:
        from ultralytics.utils import DEFAULT_CFG
        ...
        def capture(losses, *, node_losses=None, node_tensors=None):
            captured["losses"] = losses
            captured["node_losses"] = node_losses
            captured["node_tensors"] = node_tensors
            return None

        model.observe_gradient_diagnostics = capture
        model.loss(batch)

        losses = captured["losses"]
        node_losses = captured["node_losses"]
        self.assertIsInstance(losses, dict)
        self.assertIsInstance(node_losses, dict)
        self.assertEqual(
            set(node_losses),
            {
                "detection_base",
        ...
        reconstructed = (
            node_losses["detection_base"]
            + node_losses["dcli_conditioning"]
            + node_losses["dcli_calibration"]
        )
        self.assertTrue(
            torch.equal(reconstructed, losses["detection"])
        )
