...
SPECS = (
    EvidenceSpec(
        "joint_forward_validation_e90_s17",
        "ifdr",
        "ifdr-protected-counterfactual-joint-e90",
        17,
        Path(
            "configs/experiments/mechanisms/"
            "kitti_ifdr_protected_counterfactual_joint_e90_s17.yaml"
        ),
        expected_epochs=90,
    ),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate joint paired forward with recoverable training."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"JOINT FORWARD VALIDATION {state}")
