...
REGISTERED_P2_PATH_NODES = (17,)


@dataclass(frozen=True)
class P2PathSwitches:
    """Fail-closed switches for the registered final P2 interaction paths."""

    nodes: tuple[int, ...] = REGISTERED_P2_PATH_NODES
    fusion_modulation: bool = True
    dcli_factor_conditioning: bool = True

    def __post_init__(self) -> None:
        if self.nodes != REGISTERED_P2_PATH_NODES:
            raise ValueError("p2 path switches must use registered node 17")
        if not isinstance(self.fusion_modulation, bool):
            raise ValueError("fusion_modulation must be a boolean")
        if not isinstance(self.dcli_factor_conditioning, bool):
            raise ValueError("dcli_factor_conditioning must be a boolean")

    @property
    def identity(self) -> dict[str, object]:
        return {
            "nodes": self.nodes,
            "fusion_modulation": self.fusion_modulation,
            "dcli_factor_conditioning": self.dcli_factor_conditioning,
        }


def _coerce_p2_path_switches(value: object) -> P2PathSwitches:
    if value is None:
        return P2PathSwitches()
    if isinstance(value, P2PathSwitches):
        return value
    if not isinstance(value, Mapping):
        raise ValueError("p2 path switches must be a registered mapping")
    expected = {
        "nodes",
        "fusion_modulation",
        "dcli_factor_conditioning",
    }
    unknown = set(value) - expected
    if unknown:
        raise ValueError(f"unknown p2 path switch fields: {sorted(unknown)}")
    raw_nodes = value.get("nodes", REGISTERED_P2_PATH_NODES)
    if isinstance(raw_nodes, list):
        raw_nodes = tuple(raw_nodes)
    return P2PathSwitches(
        nodes=raw_nodes,
        fusion_modulation=value.get("fusion_modulation", True),
        dcli_factor_conditioning=value.get("dcli_factor_conditioning", True),
    )
...
class IFDRDetectionModel(DetectionModel):
    def __init__(
        ...
        *,
        ...
        p2_path_switches: P2PathSwitches | Mapping[str, object] | None = None,
        fusion_specs: tuple[
            FusionNodeSpec,
            ...,
        ] = DEFAULT_P2_FUSION_SPECS,
        ...
        self._p2_path_switches = P2PathSwitches()
        self.set_p2_path_switches(p2_path_switches)
    ...
    @property
    def p2_path_switches(self) -> P2PathSwitches:
        return self._p2_path_switches

    def set_p2_path_switches(
        self,
        value: P2PathSwitches | Mapping[str, object] | None,
    ) -> None:
        switches = _coerce_p2_path_switches(value)
        registered = set(self._fusion_node_indices)
        if not set(switches.nodes).issubset(registered):
            raise ValueError("p2 path switches require registered node 17")
        for index in switches.nodes:
            layer = self.model[index]
            if not isinstance(layer, ReliabilityGatedConcat):
                raise ValueError(f"p2 path switch node {index} is not registered")
            layer.set_fusion_modulation(switches.fusion_modulation)
        self._p2_path_switches = switches
