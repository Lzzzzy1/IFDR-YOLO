...
def _fresh_ultralytics_callbacks() -> dict[str, list[Callable[[Any], None]]]:
    from ultralytics.utils.callbacks.base import get_default_callbacks

    return {event: list(callbacks) for event, callbacks in get_default_callbacks().items()}
...
class IFDRPreparedHandle:
    model: Any
    model_path: Path
    callbacks: dict[str, list[Callable[[Any], None]]] = field(default_factory=_fresh_ultralytics_callbacks)
    ...
    def add_callback(self, event: str, callback: Callable[[Any], None]) -> None:
        if not isinstance(event, str) or not event or not callable(callback):
            raise ValueError("callback event and callable are required")
    ...
    def prepare_model(
        ...
        *,
        ...
        model = self._model_factory(
            ...
            p2_path_switches=dict(method.p2_path_switches.identity),
        )
        ...
        payload.update(
            {
                ...
                "p2_path_switches": method.p2_path_switches.identity,
            }
        )
