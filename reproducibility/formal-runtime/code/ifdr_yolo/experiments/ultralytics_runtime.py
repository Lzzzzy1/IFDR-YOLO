...
def bootstrap_ultralytics_config(
    ...
    *,
    ...
    registered = environment.get("YOLO_CONFIG_DIR")
    if registered:
        candidate = Path(registered)
        if not candidate.is_absolute():
            raise RuntimeError("registered YOLO_CONFIG_DIR must be absolute")
        config_dir = candidate.resolve()
        if config_dir == root or root in config_dir.parents:
            raise RuntimeError("registered YOLO_CONFIG_DIR must be outside the immutable repository root")
    else:
        root_identity = hashlib.sha256(str(root).encode("utf-8")).hexdigest()[:16]
        config_dir = (
