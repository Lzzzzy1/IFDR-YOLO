...
@dataclass(frozen=True)
class IFDRP2PathSwitchesConfig:
...
class IFDRMethodConfig:
    ...
    p2_path_switches: IFDRP2PathSwitchesConfig = IFDRP2PathSwitchesConfig()

    @property
    def identity(self) -> dict[str, object]:
        return {"p2_path_switches": self.p2_path_switches.identity}
...
def _parse_ifdr_p2_path_switches(
    value: object,
) -> IFDRP2PathSwitchesConfig:
    mapping = _require_mapping(value, "ifdr.p2_path_switches")
    expected = {
        "nodes",
        "fusion_modulation",
    ...
    _require_fields(
        mapping,
        field="ifdr.p2_path_switches",
        expected=expected,
    )
    raw_nodes = mapping["nodes"]
    if not isinstance(raw_nodes, (list, tuple)):
        raise ValueError(
            "ifdr.p2_path_switches must use registered node 17"
        )
    nodes = tuple(
        _require_int(node, "ifdr.p2_path_switches.nodes[]", minimum=0)
        for node in raw_nodes
    )
    if nodes != REGISTERED_P2_PATH_NODES:
        raise ValueError(
            "ifdr.p2_path_switches must use registered node 17"
        )
    return IFDRP2PathSwitchesConfig(
        nodes=nodes,
        fusion_modulation=_require_bool(
            mapping["fusion_modulation"],
            "ifdr.p2_path_switches.fusion_modulation",
        ),
        dcli_factor_conditioning=_require_bool(
            mapping["dcli_factor_conditioning"],
            "ifdr.p2_path_switches.dcli_factor_conditioning",
        ),
    )
...
def _parse_ifdr_method(value: object) -> IFDRMethodConfig:
    ...
    _require_fields(
        ...
        optional={"gradient_diagnostic_interval", "p2_path_switches"},
    )
    ...
    return IFDRMethodConfig(
        ...
        p2_path_switches=(
            _parse_ifdr_p2_path_switches(mapping["p2_path_switches"])
            if "p2_path_switches" in mapping
            else IFDRP2PathSwitchesConfig()
        ),
    )
