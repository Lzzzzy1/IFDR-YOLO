...
- 2,000 image-cluster bootstrap replicates with seed `20260804` and percentile 95% intervals. Multinomial image weights reproduce sampling source images with replacement while keeping every object, seed, and reliability node from the same source image in one cluster. Each replicate recomputes exact weighted average-tie ranks from precomputed value-group memberships; raw Spearman uses weighted rank moments, and residual Spearman refits the registered weighted least-squares controls using the replicate-specific target, prediction, and height ranks. This is algebraically equivalent to expanding the sampled image clusters and reranking them, but avoids rebuilding row objects or sorting hundreds of thousands of values in every replicate.
